build-iso:
	cp -v source/gogx_krnl.elf limine.cfg limine/limine.sys limine/limine-cd.bin limine/limine-cd-efi.bin iso.f16 iso_root/
...
every:
	cd source && make clean && make && cd .. && make && make run
